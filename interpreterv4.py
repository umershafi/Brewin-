...
class Interpreter(InterpreterBase):
    ...
    def __call_func(self, call_ast):
        ...
        self.env.push()
        added_parameters = self.__prepare_params(target_ast, call_ast)
        self.__prepare_env_with_closed_variables(target_closure, added_parameters)
    ...
    def __call_method(self, call_ast):
        ...
        object_val_obj = self.env.get(obj_name)
        if object_val_obj is None:
            super().error(ErrorType.NAME_ERROR, f"Variable {obj_name} not found")
        if object_val_obj.type() != Type.OBJECT:
            super().error(ErrorType.TYPE_ERROR, "Trying to call method on non-object")

        # by here we know that we have a valid object left of the dot.
        obj = object_val_obj.value()
        closure_val_obj = obj.get(method_name)
        if closure_val_obj is None:
            super().error(
                ErrorType.NAME_ERROR, f"Method {obj_name}.{method_name} not found"
            )
        if closure_val_obj.type() != Type.CLOSURE:
            super().error(ErrorType.TYPE_ERROR, "Trying to call non-function/closure")

        # by here we have a valid closure
        target_closure = closure_val_obj.value()
        num_formal_params = len(target_closure.func_ast.get("args"))
        if num_formal_params != len(args):
            super().error(
                ErrorType.NAME_ERROR,
                f"Invalid # of args to method {obj_name}.{method_name}",
            )
        ...
        if obj_name != "this":
            self.env.set("this", object_val_obj, True)

        
        self.__prepare_env_with_closed_variables(target_closure)
        self.__prepare_params(target_ast, call_ast)
    ...
    def __prepare_env_with_closed_variables(self, target_closure, added_parameters=[]):
        for var_name, value in target_closure.captured_env:
            ...
            if var_name in added_parameters:
                continue
            self.env.set(var_name, value, True)

    def __prepare_params(self, target_ast, call_ast):
        actual_args = call_ast.get("args")
        ...
        added_parameters = []
        params_to_add_to_env = {}
        for formal_ast, actual_ast in zip(formal_args, actual_args):
            if formal_ast.elem_type == InterpreterBase.REFARG_DEF:
                result = self.__eval_expr(actual_ast, is_ref=True)
            else:
                result = copy.deepcopy(self.__eval_expr(actual_ast))
            arg_name = formal_ast.get("name")
            # self.env.create(arg_name, result)
            params_to_add_to_env[arg_name] = result
            added_parameters.append(arg_name)
        for arg_name, result in params_to_add_to_env.items():
            self.env.create(arg_name, result)
    ...
    def __assign(self, assign_ast):
        ...
        # handle obj.field assignment case
        if "." in var_name:
            name_dot_name = var_name.split(".")
            target_value_obj = self.env.get(name_dot_name[0])
            self.__check_obj_name_and_type(name_dot_name[0], target_value_obj)
            target_obj = target_value_obj.value()
            if (
                src_value_obj.type() != Type.OBJECT
                and src_value_obj.type() != Type.NIL
                and name_dot_name[1] == "proto"
            ):
                super().error(
                    ErrorType.TYPE_ERROR,
                    "Can't set proto to non-object type",
                )
            if target_obj.get(name_dot_name[1]) is None or not target_obj.from_self(name_dot_name[1]):
                target_obj.set(name_dot_name[1], src_value_obj)
            else:
                target_obj.get(name_dot_name[1]).t = src_value_obj.t
                target_obj.get(name_dot_name[1]).v = src_value_obj.v
            return
        ...
        if target_value_obj is None:
            ...
            # check if any object's proto is going to be changed to a non-object type
            if target_value_obj.t == Type.OBJECT and src_value_obj.t != Type.OBJECT:
                for temp_env in reversed(self.env.environment):
                    for temp_obj in temp_env.values():
                        if temp_obj.t == Type.OBJECT and 'proto' in temp_obj.v.dict and temp_obj.v.dict['proto'] == target_value_obj:
                            super().error(
                                ErrorType.TYPE_ERROR,
                                "Can't set proto to non-object type",
                            )
    ...
    def __eval_expr(self, expr_ast, is_ref=False):
        ...
        if expr_ast.elem_type == InterpreterBase.VAR_DEF:
            return self.__eval_name(expr_ast, is_ref=is_ref)
        ...
        if expr_ast.elem_type == InterpreterBase.OBJ_DEF:
            return create_value(InterpreterBase.OBJ_DEF)

    def __eval_name(self, name_ast, is_ref=False):
        var_name = name_ast.get("name")

        # handle field case
        if "." in var_name:
            name_dot_name = var_name.split(".")
            val_obj = self.env.get(name_dot_name[0])
            self.__check_obj_name_and_type(name_dot_name[0], val_obj)
            obj = val_obj.value()
            field_value_obj = obj.get(name_dot_name[1])
            if field_value_obj is None:
                super().error(
                    ErrorType.NAME_ERROR,
                    f"Field {var_name} not found in {var_name}",
                )
            return field_value_obj

        # handle non-field case
        if not is_ref:
    ...
    def __check_obj_name_and_type(self, var_name, val_obj):
        if val_obj is None:
            super().error(
                ErrorType.NAME_ERROR,
                f"Variable {var_name} not found",
            )
        if val_obj.type() != Type.OBJECT:
            super().error(
                ErrorType.TYPE_ERROR,
                f"Dot operator used on non-object {var_name}",
            )
